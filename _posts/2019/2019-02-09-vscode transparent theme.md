...
title: VS Code 透明主题
categories: [vscode]
description: VS Code 透明主题
keywords: vs-code
---

最近很流行 vscode 透明主题, 感觉很不错, 但是还有很多小伙伴都没有用上, 所谓独乐乐不如众乐乐, 所以我就来分享一下本人配置主题的过程, 和其中遇到的问题.

## 第一步

下载插件 [Custom CSS and JS Loader](vscode:extension/be5invis.vscode-custom-css)

## 第二步

随便找一个文件夹创建文件 `custom.css`、`custom.js` 文件

在 `custom.css` 中写入以下代码（如果只需要一种效果可以只复制一部分）
...
> 此处为 macos 下的 vscode 设置 vibrancy 属性, windows 中暂不支持, windows 想实现透明主题可以直接搜索插件--[GlassIt-VSC](https://marketplace.visualstudio.com/items?itemName=s-nlf-fh.glassit)
...
打开 vscode 设置页面, 在 `settings.json` 中添加以下代码

```js
"vscode_custom_css.imports": [
  "file:///Users/xxx-path/custom.css",
  "file:///Users/xxx-path/custom.js"
],
"vscode_custom_css.policy": true
```

其中 `xxx-path` 为文件的具体路径, 可以通过将文件拖拽到终端来获取文件的路径

## 第四步

在 vscode 中按快捷键 command + shift + p(ctrl + shift + p), 输入 reload custom css and js, 然后确认

然后点击右下角弹窗重启 vscode

如果重启后 vscode 提示损坏错误，点击弹窗上的齿轮 -> 不再显示

## 最后效果就是这样

![](/assets/images/posts/vscode/16aa0a8fd1b6c9f9.webp)
...
- [vscode透明主题、霓虹灯字体 - 掘金](https://juejin.im/post/5cd51d73f265da03a97b06ef#heading-3)
- [给你的 vscode 换上透明主题 - AXIS](https://blog.axis-studio.org/2019/05/11/%E7%BB%99%E4%BD%A0%E7%9A%84vscode%E6%8D%A2%E4%B8%8A%E9%80%8F%E6%98%8E%E4%B8%BB%E9%A2%98/)
