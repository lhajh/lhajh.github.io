...
# node
## 安装 node
- 在 Nodejs [官网](https://nodejs.org/en/#download)下载 node (现在最新的是 8.x，一般使用稳定版 6.x)
- Node.js 在 Windows 平台上提供了两种安装方式：一是 .MSI 安装文件，另外还有一个 .EXE 可执行文件。我选择了 .EXE 文件 v6.11.3 LTS 版本
- 先在电脑创建 D:\nodejs 目录，并将 Node.exe 安装保存在这个目录中，前辈说装在C盘会有权限的问题
